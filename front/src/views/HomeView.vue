  ...
  <div class="book-page">
    ...
    <div class="book-list">
      <div class="book-card" v-for="(book, index) in books" :key="index">
        ...
        <h2>{{ book.title }}</h2>
        <p>{{ book.author }}</p>
        <button @click="deleteBook(book._id)">Supprimer</button>
        <button @click="goToBookPage(book._id)">Détails</button>
      </div>
    </div>

    <modal ref="addBookModal" width="90%">
      <template #header=""> Ajout d'un nouveau livre </template>
      <template #body="">
        <div>
          <form id="book-form">
...
const reload = async () => {
  const {data: _books} = await api.get('/v1/books')
  books.value = _books
}
onMounted(reload)

const addBook = async () => {
  addBookModal.value.open().subscribe(async (res) => {
    if (!res) {
      newBook.value = new Book({})
      return
    }
    await api.post('/v1/books', newBook.value)
    await reload()
  })
}
